        ...
        return [
            ...
            Grid::make(10)
                ->schema([
                    ...
                    // DEPLOY para tipo de proceso y sus plazos
                    TextInput::make('tipo_proceso') // cambiamos nombre para no validar con el campo real 'process_type_id'
                        ...
                        ->schema([
                            DatePicker::make('s2Stage.published_at')
                                ...
                                ->afterStateUpdated(function ($state, $set, $get, $record) {
                                    $rule = TenderDeadlineRule::active()
                                        ->where('from_field', 's2Stage.published_at')
                                        ->where('to_field', 's2Stage.participants_registration')
                                    ...
                                    if ($rule && $state) {
                                        ...
                                        // Llamada correcta al método estático de la misma clase
                                        $participantsDate = self::addBusinessDays($date, $rule->legal_days);

                                        $set('s2Stage.participants_registration', $participantsDate->format('Y-m-d'));
                                    }
                                })
                                ->visible(fn ($record) => $record?->s2Stage)
